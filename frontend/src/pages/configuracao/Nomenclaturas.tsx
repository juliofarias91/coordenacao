/** SP-104 · Nomenclaturas & padrões — o padrão de nome do projeto.
 *
 *  O padrão oficial: vive no backend, vale para o time todo e é o que o
 *  validador da Fase 3 (SP-301) consome para dizer, campo a campo, onde o nome
 *  divergiu — e o que gera penalidade, notificação e trilha.
 *
 *  ═══ ERAM QUATRO ABAS, DE DUAS ORIGENS. SOBROU ESTA (07/08/2026, a pedido)
 *
 *  `Auditoria de arquivos`, `Padrões avançados` e `Palavras aceitas` vieram do
 *  **Auditer** e rodavam inteiras no navegador, sem tocar na API: elas auditavam
 *  a PASTA (PDF de spec, planilha de controle, relatório), enquanto esta audita o
 *  MODELO entregue. **O módulo inteiro foi removido**, não só as abas — o motor
 *  em `lib/auditer/`, o corretor ortográfico em WebAssembly, os dicionários de
 *  5,7 MB e as três dependências que só ele usava (`xlsx`, `hunspell-asm`,
 *  `dictionary-pt`/`-en`).
 *
 *  O QUE SE PERDEU, para quem for procurar: padrão com separador por bloco e
 *  segmento tipado (data/número/texto), duplicidade por CONTEÚDO (SHA-256 — a
 *  cópia salva com outro nome), higiene de nome e ortografia de planilha. Nada
 *  disso existe do lado do servidor. Se voltar a fazer falta, está inteiro no
 *  histórico: `git log -- frontend/src/lib/auditer/`.
 *
 *  Com uma seção só, o `Segmented` saiu junto — segmento de um item é um botão
 *  que não faz nada. E o invólucro que existia só para desenhá-lo saiu também:
 *  esta tela ERA `nomenclatura/PadraoDoProjeto.tsx`, filha de um
 *  `Nomenclaturas.tsx` de oitenta linhas. Uma seção, um arquivo, como todas as
 *  outras da configuração.
 */
